use crate::config::structs::{Config, HostConfig, HostInputKind, VerboseLevel};
...
pub async fn deliver(
    ...
    let mail_body = load_templates().get(&host_config.language)
        .ok_or_else(|| {
            ...
            throw(EK::EmailLanguageNotFound, format!("defined language not found for {host_name} ({:?})", &host_config.language))
        })?
        .to_string();

    let form_data = fill_form_data(host_config, form_values).inspect_err(|_| {
        dbg_print_form(config, form_values.clone());
    })?;
    ...
    let subject = mail_body.lines().next()
        .ok_or_else(|| {
            dbg_print_form(config, form_values.clone());
            throw(EK::EmailLinesFail, format!("Could not read next line for {host_name}, {mail_body:?}"))
        })?;
    ...
    let mailer: AsyncSmtpTransport<Tokio1Executor> =
        AsyncSmtpTransport::<Tokio1Executor>::relay(&config.mail.mailserver_address)
            .map_err(|e| {
                dbg_print_form(config, form_values.clone());
                throw(EK::EmailHostnameReadFail, format!("Could not read mailserver hostname {e}"))
            })?
            .credentials(creds)
            .build();
    ...
    mailer
        ...
        .map_err(|e| {
            dbg_print_form(config, form_values.clone());
            throw(EK::EmailSendFail, e.to_string())
        })?;

    if config.general.verbose_level == VerboseLevel::Info {
        println!("Form sent successfully to {} at {}", host_config.recipient, Utc::now().format("%+"));
    }
