...
use crate::{Config, DbConn, DbPool, PooledDbConn};
...
impl Star {
    pub fn insert_bulk(conn: &mut DbConn, i_stars: &[NewStar]) -> Result<usize, ServerError> {
        ...
        diesel::insert_into(table)
            .values(i_stars)
            .execute(conn)
            .map_err(|e| throw(ErrorKind::DbFail, e.to_string()))
    }
}

impl PublicStar {
    pub fn get_all_public(conn: &mut DbConn) -> Result<Vec<PublicStar>, ServerError> {
        use crate::db::schema::star::{id, position_x, position_y, startype, table as s};
        use crate::db::schema::transaction::{day, message, table as t, username};
...
impl OwnStar {
    ...
    pub fn get_from_token(
        conn: &mut DbConn,
        i_token: &str,
    ) -> Result<Option<OwnStarWithId>, ServerError> {
        use crate::db::schema::transaction::{
            gems, id, is_token_used, message, table, token, username,
        };
...
impl Transaction {
    pub fn find_by_haid(
        ...
        use crate::db::schema::transaction::{ha_id, table};
        table
            .filter(ha_id.eq(i_haid))
    ...
    pub fn update(conn: &mut DbConn, i_tr: &Transaction) -> Result<Transaction, ServerError> {
        use crate::db::schema::transaction::{id, table};
    ...
    pub fn toggle_check(conn: &mut DbConn, i_id: i32) -> Result<Transaction, ServerError> {
        use crate::db::schema::transaction::{id, is_checked, table};
    ...
    pub fn send_mail(conn: &mut DbConn, i_id: i32) -> Result<Transaction, ServerError> {
        use crate::db::schema::transaction::{id, is_mail_sent, table};
    ...
    pub fn update_with_stars(
        conn: &mut DbConn,
        i_id: i32,
        star_post: OwnTokenPost,
    ) -> Result<Transaction, ServerError> {
        use crate::db::schema::transaction::{id, is_token_used, message, table, username};

        diesel::update(table)
            .filter(id.eq(i_id))
            .set((
                is_token_used.eq(true),
                username.eq(star_post.username),
                message.eq(star_post.message),
            ))
            .get_result::<Transaction>(conn)
            .map_err(|e| throw(ErrorKind::DbFail, e.to_string()))
    }
