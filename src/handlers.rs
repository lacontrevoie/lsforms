...
use crate::config::structs::{Config, HostInput, HostInputKind};
use crate::config::methods::{load_config, load_hosts, load_re_email};
use crate::errors::{throw, ErrorKind as EK, ServerError, dbg_print_form};
...
pub async fn get_captcha(form_host: web::Path<String>) -> Result<HttpResponse, ServerError> {
    ...
    let (host_name, host_config) = hosts
        .get_key_value(&host)
        .ok_or_else(|| {
            throw(EK::UnknownHost, format!("no host named {host}"))
        })?;

    // check if captcha is enabled for this host
    true.then_some(host_config.enable_captcha)
        .ok_or_else(|| {
            throw(EK::CaptchaDisabled, format!("captcha is disabled for host {host_name}"))
        })?;
    ...
    let challenge = create_challenge(ChallengeOptions {
        ...
        throw(EK::CaptchaGenerationFailed, format!("could not generate challenge for {host_name}"))
    })?;
...
pub async fn post_form(form_host: web::Path<String>, form: web::Form<Value>, req: HttpRequest) -> Result<HttpResponse, ServerError> {
    ...
    for field in &host_config.inputs {
        form_input_check(config, re_email, field, &form_values)?;
    }
    ...
    let client_ip = client_connection.realip_remote_addr()
        .ok_or_else(|| {
            dbg_print_form(config, form_values.clone());
            throw(EK::NoClientIP, format!("could not get client IP for {host}"))
        })?;
...
fn form_input_check(config: &Config, re_email: &Regex, host_field: &HostInput, form_values: &HashMap<String, String>) -> Result<(), ServerError> {
    ...
    let form_field = match form_values.get(&host_field.name) {
        ...
        Some(f) if f.is_empty() && host_field.required => {
            dbg_print_form(config, form_values.clone());
            return Err(throw(EK::FieldRequiredButEmpty, format!("field {} is empty", host_field.name)))
        },
        ...
        None => {
            dbg_print_form(config, form_values.clone());
            return Err(throw(EK::FieldRequiredButMissing, format!("missing field {}", host_field.name)))
        },
    };
    ...
    if host_field.kind == HostInputKind::Select {
        let select_options = host_field.settings.options.clone()
            .ok_or_else(|| {
                dbg_print_form(config, form_values.clone());
                throw(EK::FieldSelectNoOptions, format!("select {} has no options", host_field.name))
            })?;
...
fn form_to_hashmap(form_data: &Value) -> Result<HashMap<String, String>, ServerError> {
    ...
    let form_data_obj = form_data.as_object()
        .ok_or_else(|| {
            throw(EK::FormDataNotObject, format!("Received form data is not convertible to object: {form_data:?}"))
        })?;

    for (key, value) in form_data_obj {
        let value_str = value.as_str()
        .ok_or_else(|| {
            throw(EK::FormParamNotString, format!("Could not read form param as string: {form_data:?}"))
        })?
        .to_string();
